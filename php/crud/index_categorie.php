...
<body>
    ...
    <div class="container">
        ...
        <form action="index_categorie.php" method="get">
            Mot clé : <input type="search" name="motcle" autocomplete="off" value="<?php
                                                                                    if (isset($_GET['motcle']) && !empty($_GET['motcle'])) {
                                                                                        echo $_GET['motcle'];
                                                                                    }
                                                                                    ?>">
            ...
            <tbody>
                ...
                <?php foreach ($categories as $p) { ?>
                    <tr>
                        ...
                        <td><?= $p['nom'] ?></td>
                        <td> <a onclick="return confirm('Attention : la suppression d\'une categorie entraine la suppression en cascade de tous ses produits. Supprimer ?')" href="delete_categorie.php?id=<?= $p['id'] ?>" class="btn btn-sm btn-danger">Supprimer</a>
                            <a href="edit_categorie.php?id=<?= $p['id'] ?>" class="btn btn-sm btn-warning">Modifier</a>
                            <a href="show_categorie.php?ounacer=<?= $p['id'] ?>" class="btn btn-sm btn-info">Consulter</a></td>
                    </tr>
                <?php } ?>
            </tbody>
